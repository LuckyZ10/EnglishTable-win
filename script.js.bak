...
async function removeDuplicateRecords() {
  try {
    showLoading('正在检测和删除重复记录...');
    
    // 加载所有记录
    await loadStudyRecords();
    
    if (studyRecords.length === 0) {
      showToast('info', '没有记录需要处理');
      hideLoading();
      return;
    }
    ...
    showToast('error', '删除重复记录失败：' + error.message);
  } finally {
    hideLoading();
  }
}
...
async function handleImport() {
  ...
  if (!file) {
    showToast('warning', '请选择要导入的CSV文件');
    return;
  }
  ...
  try {
    ...
    if (records.length === 0) {
      showToast('warning', 'CSV文件中没有可导入的记录');
      hideLoading();
      return;
    }
    ...
    const newRecords = records.map((record, index) => ({
      ...
      translation: record['中文翻译'] || record.translation || '',
      example: record['应用示例'] || record.example || '',
      note: (record['学习笔记'] || record.note) || '',
      timestamp: record['时间戳'] || record.timestamp || new Date().toISOString()
    }));
    ...
    if (uniqueNewRecords.length === 0) {
      showToast('info', '所有记录都是重复的，没有导入新记录');
      hideLoading();
      hideImportModal();
      return;
    }
    ...
    // 显示成功提示
    showToast('success', `成功导入 ${uniqueNewRecords.length} 条记录（过滤掉 ${newRecords.length - uniqueNewRecords.length} 条重复记录）`);
  } catch (error) {
...
function bindEventListeners() {
  ...
  // 回车键添加记录
  wordInput.addEventListener('keypress', async function(e) {
    if (e.key === 'Enter') {
      try {
        await addRecord();
      } catch (error) {
...
exportBtn.addEventListener('click', showExportModal);
  ...
  // 为所有表格行添加点击事件（委托）
  recordsTable.addEventListener('click', async function(e) {
    // 处理复选框点击
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    showToast('error', '应用初始化失败，请刷新页面重试');
  }
});
